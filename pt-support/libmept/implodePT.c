...
static ATerm implodeTerm(ATerm);
static ATerm implodeLayout(ATerm t);
static ATerm implodeLexical(ATerm t);
static ATerm implodeVar(ATerm t);
static ATerm implodeProd(ATerm prod);
...
static ATermList implodeArgs(ATermList tl)
{
  ...
  newTerm = implodeTerm(ATgetFirst(tl));
  if (newTerm) {
    return ATinsert(implodeArgs(ATgetNext(tl)), newTerm);
  }
  else {
    return implodeArgs(ATgetNext(tl));
  }
}
...
static ATermList implodeLexicalList(ATerm t, ATermList tail)
{
  ATerm     prod;
  ATermList list;

  if (!ATmatchTerm(t, asfix_appl_pattern, &prod, &list)) {
    ATerror("implodeLexicalList: not an appl pattern: %t\n", t);
    return NULL;
  } 
  
  if (!isLexicalListProd(prod)) {
    ATerm newTerm;
    newTerm = implodeTerm(t);
    if (newTerm) {
  ...
  if (ATgetLength(list) == 1) {
    return implodeLexicalList(ATgetFirst(list), tail);
  }
  ...
  if (ATgetLength(list) == 2) {
    ...
    tail = implodeLexicalList(rightBranch, tail);
    tail = implodeLexicalList(leftBranch, tail);

    return tail;
  }
  
  ATerror("implodeLexicalList: illegal list: %t\n", t);
  return NULL;
}
...
static ATermList implodeCharClassList(ATerm t, ATermList tail)
{
  ATerm     prod;
  ATermList list;
  ...
  if (!ATmatchTerm(t, asfix_appl_pattern, &prod, &list)) {
    ATerror("implodeCharClassList: not an appl pattern: %t\n", t);
  }
  ...
  if (ATgetLength(list) == 1) {
    return implodeCharClassList(ATgetFirst(list), tail);
  }
  ...
  if (ATgetLength(list) == 2) {
    ...
    tail = implodeCharClassList(rightBranch, tail);
    tail = implodeCharClassList(leftBranch, tail);

    return tail;
  }                 

  ATerror("implodeCharClassList: illegal list: %t\n", t);
  return NULL;
}
...
static ATermList implodeList(ATerm t, ATermList tail)
{
  ATerm     prod;
  ATermList list;

  if (!ATmatchTerm(t, asfix_appl_pattern, &prod, &list)) {
    ATerror("implodeList: not an appl pattern: %t\n", t);
    return NULL;
  }

  if (!isListProd(prod)) {
    ATerm newTerm = implodeTerm(t);
    if (newTerm) {
      return ATinsert(tail, newTerm);
    }
    else {
  ...
  if (ATgetLength(list) == 1) {
    return implodeList(ATgetFirst(list), tail);
  }                                    
  ...
  if (ATgetLength(list) == 3) {
    ...
    tail = implodeList(rightBranch, tail);
    if (!remove_layout) {
      tail = ATinsert(tail, implodeLayout(layoutAfterLeft));
    }
    tail = implodeList(leftBranch, tail);
  
    return tail;
  }

  ATerror("implodeList: illegal list: %t\n", t);
  return NULL;
}
...
static ATermList implodeSepList(ATerm t, ATermList tail)
{
  ATerm     prod;
  ATermList list;

  if (!ATmatchTerm(t, asfix_appl_pattern, &prod, &list)) {
    ATerror("implodeSepList: not an appl pattern: %t\n", t);
    return NULL;
  }
  ...
  if (ATgetLength(list) == 1) {
    return implodeSepList(ATgetFirst(list), tail);
  }
  ...
  if (ATgetLength(list) == 5) {
    ...
    tail = implodeSepList(rightBranch, tail);
    if (!remove_layout) {
      tail = ATinsert(tail, implodeLayout(layoutAfterSep));
    } 
    if (!remove_literals) {
      tail = ATinsert(tail, implodeTerm(separator));
    }
    if (!remove_layout) {
      tail = ATinsert(tail, implodeLayout(layoutAfterLeft));
    } 
    tail = implodeSepList(leftBranch, tail);
  
    return tail;
  }

  ATerror("implodeSepList: illegal list: %t\n", t);
  return NULL;
}
...
static ATerm implodeProd(ATerm prod)
{
  ...
  if(!ATmatchTerm(prod, asfix2_prod_pattern, NULL, NULL, &attr)) {
    ATerror("implodeProd: not a production: %t\n", prod);
  }
...
static ATerm implodeLexical(ATerm t)
{
  ...
  if (flatten_lexical) {
    ...
    if (lexical_to_string) {
      ...
      return ATmakeTerm(asfix_appl_pattern, implodeProd(outerProd), newArgs);
    }
  }
  else { 
    return t;
...
static ATermList implodeLayoutList(ATermList args, ATermList tail)
{
  ATerm     arg, prod;
  ATermList list;
  ...
  while (!ATisEmpty(args)) {
    ...
    if (ATgetType(arg) == AT_INT) {
      ...
      if (ATmatchTerm(prod, asfix2_prod_lex_layout_pattern, NULL, NULL)
        ...
        tail = implodeLayoutList(ATreverse(list), tail);
      }
      else {
        ATerm newArg = implodeTerm(arg);
        if (newArg) {
...
static ATerm implodeLayout(ATerm t)
{
  ...
  if (flatten_layout) {
    if (ATmatchTerm(t, asfix_appl_pattern, NULL, &args)) {
      ATermList newArgs = implodeLayoutList(args, ATempty);
    ...
    ATerror("implodeLayout: not an appl pattern: %t\n\n", t);
    return NULL;
  }
  else {
    return t;
...
static ATerm implodeLiteral(ATerm prod)
{
  char *lit;
  ...
  ATerror("implodeLiteral: not a literal prod: %t\n", prod);
  return NULL;
}
      ...
      if (ATmatchTerm(arg, asfix_appl_pattern, &prodVarsymInner, &argsInner)) {
        if (ATmatchTerm(prodVarsymInner, asfix2_prod_varsym_pattern,
          ...
          newVarArg = ATmakeList1(ATmakeTerm(asfix_appl_pattern,
                                             implodeProd(prodVarsymInner),
                                             newVarArgs));
          return ATmakeTerm(asfix_appl_pattern,
                            implodeProd(prodVarsymOuter),
                            newVarArg);
        }
      }
    }
  }
  ATerror("implodeVar: not an appl pattern: %t\n\n", t);
  return NULL;
}

static ATerm implodeTerm(ATerm t)
{
  ATerm prod;
  ATermList args;
  ...
  if (flatten_list) {
    if (isSepListProd(prod)) {
      ATermList newList = ATmakeList1((ATerm)implodeSepList(t, ATempty));
      return ATmakeTerm(asfix_appl_pattern, implodeProd(prod), newList);
    }

    if (isListProd(prod)) {
      ATermList newList = ATmakeList1((ATerm)implodeList(t, ATempty));
      return ATmakeTerm(asfix_appl_pattern, implodeProd(prod), newList);
    }

    if (isLexicalListProd(prod)) {
      return (ATerm)implodeLexicalList(t, ATempty);
    }

    if (isCharClassListProd(prod)) {
      return (ATerm)implodeCharClassList(t, ATempty);
    }
  }

  /* Default: application */
  return ATmakeTerm(asfix_appl_pattern, implodeProd(prod), implodeArgs(args));
}

static ATerm implodeParseTree(ATerm tree)
{
  ATerm  newTree, ambCnt;

  if (ATmatchTerm(tree, asfix2_parsetree_pattern, &newTree, &ambCnt)) {
    return ATmakeTerm(asfix2_parsetree_pattern, implodeTerm(newTree), ambCnt);
  }
...
ATerm implodePT(ATerm t)
{
  ATerm newTree;

  init_a2toa1_patterns();
  newTree = implodeParseTree(t);
  return newTree;
}
