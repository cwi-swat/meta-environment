...
/**
 * \file
 * Flattening is an essential phase after parsing that maps several
 * list constructors for list non-terminals (i.e. productions generated
 * by the SDF2 normalizer), to a flat representation using the list
 * production constructor. This code mirrors the SDF2 normalizer by
 * recognizing tree patterns that are a result of the regular production
 * generator in the SDF normalizer.
 *
 * Why list flattening? Since lists construction is an 'associative'
 * operation (semantically), a flat representation is an efficient
 * canonical representation when frequently matching and reconstructing
 * lists by concatenation and list matching.
 *
 * Furthermore, this phase is used to stage an incremental bootstrapping
 * process towards a more stable parse forest representation. Certain
 * ideosynchrasies of SDF2 are hidden here by transforming them. One
 * of these is the fact that lists of layout constructors are not modeled
 * using the list constructors of SDF2. Instead LAYOUT LAYOUT -> LAYOUT
 * is generated. This anomaly is fixed here by transforming!
 * \todo Eventually, SDF2 will be adapted to generated LAYOUT* instead
 * of LAYOUT? and LAYOUT LAYOUT -> LAYOUT...
 */
#include <stdlib.h>
#include <string.h>
#include <assert.h>
...
/** 
 * First, we have a lot of boilerplate, co-evolution sensitive
 * parse tree patterns to recognize the productions generated by 
 * the SDF2 normalizer.
 *
 * \todo: adapt the SDF2 normalizer to mark these productions by
 * using a production attribute. Another solution is to store this
 * information in a parse table and let the parser construct flat
 * lists directly (flat_reduce??)
 */
...
/* Patterns to deal with unflattened lists with separators */
static ATerm asfix2_empty_to_star_sep_lex = NULL;
static ATerm asfix2_single_to_plus_sep_lex = NULL;
static ATerm asfix2_plus_sep_to_star_sep_lex = NULL;
static ATerm asfix2_plus_sep_plus_sep_to_plus_sep_lex = NULL;
static ATerm asfix2_star_sep_star_sep_to_star_sep_lex = NULL;
static ATerm asfix2_star_sep_plus_sep_to_plus_sep_lex = NULL;
static ATerm asfix2_plus_sep_star_sep_to_plus_sep_lex = NULL; 
...
static void init_asfix_patterns()
{
  ...
  asfix2_empty_to_star_lex_sort  =
    ATparse("prod([],lex(iter-star(<term>)),<term>)"); 

  ATprotect(&asfix2_single_to_plus_lex_sort);
  asfix2_single_to_plus_lex_sort =
    ATparse("prod([lex(<term>)],lex(iter(<term>)),<term>)");
  ...
  asfix2_empty_to_star_lex_charclass  =
    ATparse("prod([],lex(iter-star(char-class(<term>))),<term>)"); 
  ...
  asfix2_lex_layout_to_cf_layout = 
    ATparse("prod([lex(layout)],cf(layout),<term>)");
  ...
  asfix2_single_to_plus =
    ATparse("prod([cf(<term>)],cf(iter(<term>)),<term>)");    

  ATprotect(&asfix2_plus_to_star);
  asfix2_plus_to_star =
    ATparse("prod([cf(iter(<term>))],cf(iter-star(<term>)),<term>)");   
  ...
  asfix2_empty_to_star_sep =
    ATparse("prod([],cf(iter-star-sep(<term>,<term>)),<term>)");   

  ATprotect(&asfix2_single_to_plus_sep);
  asfix2_single_to_plus_sep =
    ATparse("prod([cf(<term>)],cf(iter-sep(<term>,<term>)),<term>)");  

  ATprotect(&asfix2_plus_sep_to_star_sep);
  asfix2_plus_sep_to_star_sep =
    ATparse("prod([cf(iter-sep(<term>,<term>))],cf(iter-star-sep(<term>,<term>)),<term>)"); 
  ...
  ATprotect(&asfix2_empty_to_star_sep_lex);
  asfix2_empty_to_star_sep_lex =
    ATparse("prod([],lex(iter-star-sep(<term>,<term>)),<term>)");   

  ATprotect(&asfix2_single_to_plus_sep_lex);
  asfix2_single_to_plus_sep_lex =
    ATparse("prod([lex(<term>)],lex(iter-sep(<term>,<term>)),<term>)");  

  ATprotect(&asfix2_plus_sep_to_star_sep_lex);
  asfix2_plus_sep_to_star_sep_lex =
    ATparse("prod([lex(iter-sep(<term>,<term>))],lex(iter-star-sep(<term>,<term>)),<term>)"); 

  ATprotect(&asfix2_plus_sep_plus_sep_to_plus_sep_lex);
  asfix2_plus_sep_plus_sep_to_plus_sep_lex =
    ATparse("prod([lex(iter-sep(<term>,<term>)),<term>," \
            "lex(iter-sep(<term>,<term>))],"
	    "lex(iter-sep(<term>,<term>)),<term>)");

  ATprotect(&asfix2_star_sep_star_sep_to_star_sep_lex);
  asfix2_star_sep_star_sep_to_star_sep_lex =
    ATparse("prod([lex(iter-star-sep(<term>,<term>)),<term>," \
            "lex(iter-star-sep(<term>,<term>))]," \
            "lex(iter-star-sep(<term>,<term>)),<term>)");    

  ATprotect(&asfix2_star_sep_plus_sep_to_plus_sep_lex);
  asfix2_star_sep_plus_sep_to_plus_sep_lex =
    ATparse("prod([lex(iter-star-sep(<term>,<term>)),<term>," \
            "lex(iter-sep(<term>,<term>))],"
	    "lex(iter-sep(<term>,<term>)),<term>)");                             
  ATprotect(&asfix2_plus_sep_star_sep_to_plus_sep_lex);
  asfix2_plus_sep_star_sep_to_plus_sep_lex =
    ATparse("prod([lex(iter-sep(<term>,<term>)),<term>," \
            "lex(iter-star-sep(<term>,<term>))],"
	    "lex(iter-sep(<term>,<term>)),<term>)"); 
...
ATbool isListProd(PT_Production prod)
{
  ...
  if (ATmatchTerm((ATerm)prod, asfix2_layout_layout_to_layout, NULL )) {
    return ATtrue;
  }

  if (ATmatchTerm((ATerm)prod, asfix2_lex_layout_to_cf_layout, NULL)) {
    return ATtrue;
  }

  if (ATmatchTerm((ATerm)prod, asfix2_empty_to_star, NULL, NULL)) {
    return ATtrue;
  }

  if (ATmatchTerm((ATerm)prod, asfix2_single_to_plus, &sort1, &sort2, NULL)
      ||
      ATmatchTerm((ATerm)prod, asfix2_plus_to_star, &sort1, &sort2, NULL)) {
  ...
  if (ATmatchTerm((ATerm)prod, asfix2_empty_to_star_lex_sort, NULL, NULL)) {
    return ATtrue;
  }

  if (ATmatchTerm((ATerm)prod, asfix2_single_to_plus_lex_sort, &sort1, &sort2, NULL)
      ||
      ATmatchTerm((ATerm)prod, asfix2_plus_to_star_lex_sort, &sort1, &sort2, NULL)) {
    return ATisEqual(sort1, sort2);
  } 
  ...
  if (ATmatchTerm((ATerm)prod, asfix2_empty_to_star_sep, NULL, NULL, NULL)) {
    return ATtrue;
  }

  if (ATmatchTerm((ATerm)prod, 
                  asfix2_single_to_plus_sep, &sort1, &sort2, NULL, NULL)) {
    return ATisEqual(sort1, sort2);
  }
 ...
 if (ATmatchTerm((ATerm)prod, asfix2_empty_to_star_lex_charclass, NULL, NULL)) {
    return ATtrue;
  }

  if (ATmatchTerm((ATerm)prod, asfix2_single_to_plus_lex_charclass, &cc1, &cc2, NULL)
      ||
      ATmatchTerm((ATerm)prod, asfix2_plus_to_star_lex_charclass, &cc1, &cc2, NULL)) {
...
/**
 * Flattens the lists of a parse tree and normalizes LAYOUT derivations.
 * \param tree input parse tree
 * \returns a flattened parse tree
 */
