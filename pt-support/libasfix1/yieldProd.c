...
{                            
  if (PT_isSymbolQuotedLiteral(symbol)) {
    char *str = PT_getSymbolString(symbol);
    return strlen(str) + 2;
  }
  if (PT_isSymbolUnquotedLiteral(symbol)) {
    char *str = PT_getSymbolString(symbol);
    return strlen(str);
  }
  ...
  if (PT_isSymbolIterPlusSep(symbol)
    ...
    PT_Symbol newSymbol = PT_getSymbolSymbol(symbol);
    char *separator = PT_getSymbolSeparator(symbol);
    return lengthOfSymbol(newSymbol) + strlen(separator) + 4;
  }
