...
; <term> is a VTP tree, <module-name> is a string, SEAL is the Conf. Manag.
  ...
  (ifn (send 'debug-on? (send 'eqs-debug meta))
    (setq new-term (#:EQM:evaluate (send 'EQM-select meta (concat module-name))
                                   term
                                   ()
                                   () ))
  ; else
    (setq x (vector ()))
    (#:EQM:evaluate (send 'EQM-select meta (concat module-name))
                    term
                    (acons 'EDB x ())
                    () )
