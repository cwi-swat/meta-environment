...
; ---------------------------------------------------------------------------- ;
; $Log$
; Revision 1.1  2003/04/15 08:48:23  jurgenv
...
;   (print "abs-subject-paths = " abs-subject-paths)
    ...
    (while (nextl abs-subject-paths abs-path)
      ...
      (setq subject-tree (#:origin:tree:navigate (#:EQM:get-term tp) abs-path))
      (setq subj-org (#:origin:get-all-origins subject-tree))
;     (setq subj-org
...
;   (print "paths-to-show = " )
;   (mapc (lambda (org) (print (#:origin:vtp2path org))) paths-to-show)
...
; ---------------------------------------------------------------------------- ;
; #:origin:get-all-origins : recursively retrieve all origins from a tree.
;
(de #:origin:get-all-origins (tree)
  (let ( orgs son-org )
    (#:EQM:tree:for-all-sons
      tree
      (lambda (son)
        (setq son-org (#:origin:get-all-origins son))
        (newl orgs son-org)
      )
    )
    (newl orgs (cdr (#:EQM:tree:get-info tree 'origin)))
    (apply 'append orgs)
  )
)
; ---------------------------------------------------------------------------- ;
