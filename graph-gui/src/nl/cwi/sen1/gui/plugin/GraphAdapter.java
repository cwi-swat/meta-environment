...
public class GraphAdapter extends prefuse.data.Graph {
	public GraphAdapter(nl.cwi.sen1.graph.types.Graph graph) {
		...
		for (NodeList nodes = graph.getNodes(); !nodes.isEmpty(); nodes = nodes
			...
			if (fillColor != null) {
				pNode.setInt(GraphConstants.FILLCOLOR, ColorLib.rgb(fillColor
						.getRed(), fillColor.getGreen(), fillColor.getBlue()));
			}

			Color color = getColorAttribute(node);
			if (color != null) {
				pNode.setInt(GraphConstants.COLOR, ColorLib.rgb(color.getRed(),
	...
	static private Color getColorAttribute(Node node) {
		AttributeList attrs = node.getAttributes();
		while (!attrs.isEmpty()) {
			Attribute attr = attrs.getHead();
			if (attr.isColor()) {
	...
	static public void updateNode(VisualItem node, Attribute attr) {
		if (attr.isLabel()) {
			node.setString(GraphConstants.LABEL, attr.getLabel());
		} else if (attr.isShape()) {
			node.set(GraphConstants.SHAPE, attr.getShape());
		} else if (attr.isColor()) {
			Color color = attr.getColor();
			node.setInt(GraphConstants.COLOR, ColorLib.rgb(color.getRed(),
					color.getGreen(), color.getBlue()));
		} else if (attr.isFillColor()) {
			Color color = attr.getColor();
			node.setInt(GraphConstants.FILLCOLOR, ColorLib.rgb(color.getRed(),
					color.getGreen(), color.getBlue()));
		}
	}
}
