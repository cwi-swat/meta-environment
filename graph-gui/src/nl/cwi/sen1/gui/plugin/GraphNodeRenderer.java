...
public class GraphNodeRenderer extends LabelRenderer {

    public GraphNodeRenderer(String string) {
        super(string);
    }
    ...
    protected Shape getRawShape(VisualItem item) {
        ...
        nl.cwi.sen1.graph.types.Shape shape = (nl.cwi.sen1.graph.types.Shape) item
                .get(GraphConstants.SHAPE);
        if (shape.isDiamond()) {
    ...
    private Shape getDiamondShape(double x, double y, double width,
            double height) {
        int ix = (int) x;
        int iy = (int) y;
        int iw = (int) width;
