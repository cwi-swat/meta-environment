...
public class TreeBuilder {
			...
			if(node.getChildCount() == 0){
				...
				while(iter.hasNext()){
					...
					if(n.getString("name").equals(nodeName)){
						node = n;
						node.setInt("linesCode", countLinesCode + node.getInt("linesCode"));
						childExist = true;
					}
				}
				if(!childExist){
	...
	private Node addChildToNode(Tree tree, Node parent, String nodeName, String path, int countLinesCode, String type){
		...
		nextNode.setInt("linesCode", countLinesCode);
		return nextNode;
	}
	 ...
	 * @param linesCode LOC attribute of the file the node represents
	 * @param type the type of the node (file of directory)
	 */
	private void setRoot(Tree tree, String name, String path, int linesCode, String type){
		tree.addRoot();
	...
	private void removeNodesUpToSystem(){
		...
		tempTree = new Tree();
		setRoot(tempTree, nextNode.getString("name"), nextNode.getString("path"), nextNode.getInt("linesCode"),nextNode.getString("type"));
	...
	private void copySubtree(Node currentTreeNode, Node newTreeNode){
		...
		while(iter.hasNext()){
			...
			n.setInt("linesCode", nextChild.getInt("linesCode"));
	...
	/**
	 * sets the weight attribute of node, weight is based on the number of code lines "LOC"
	 */
	public void setTreemapWeights(){
		m_dataTree.getRoot().getTable().addColumn("weight", Double.TYPE);
		m_dataTree.getRoot().setDouble("weight", 100.0);
		setChilderenWeights(m_dataTree, m_dataTree.getRoot());
	}

	/**
	 * sets the weight of a subtree nodes with a given root recursively
	 * @param tree a tree
	 * @param node the root of the tree
	 */
	private void setChilderenWeights(Tree tree, Node node){
		Iterator<Node> iter = node.children();
		Iterator<Node> it = node.children();
		while(it.hasNext()){
			Node nextChild = it.next();
			//int sumChilderenLOC = iter.next().getParent().getInt("linesCode");
			double weight = 0;
			int sumChilderenLOC = tree.getRoot().getInt("linesCode");
			if(sumChilderenLOC >= 1000){
				weight = ((double)nextChild.getInt("linesCode")*1000) / (double)sumChilderenLOC;
			} else {
				weight = ((double)nextChild.getInt("linesCode")*100) / (double)sumChilderenLOC;
			}
			weight = new BigDecimal(""+weight).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
			nextChild.setDouble("weight", weight );
			setChilderenWeights(tree, nextChild);
		}
	}
