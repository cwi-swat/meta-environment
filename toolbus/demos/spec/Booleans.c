...
#define xPUTxHEAP(a) {*(HP++) = (ptr) (a); if (save_HP<HP) save_HP=HP;}
#define xEXPRxPUTxHEAP(a)  {*(HP++) = (ptr) (a); if (save_HP<HP) save_HP=HP;}
#define xFNSHxNODE()  {HDRS->num = HP-(HDRS->sadr);xPUSHA(HDRS--); if (save_HDRS>HDRS) save_HDRS=HDRS;}
#define xPUSHM(a)  {*(MP++) = (ptr) (a); if (save_MP<MP) save_MP=MP;}
#define xPUSHA(a)  {*AP = (ptr) (a) ; AP--; if (save_AP>AP) save_AP=AP;}

#else /* DEBUG */

#define xPUTxHEAP(a) *(HP++) = (ptr) (a); 
#define xEXPRxPUTxHEAP(a)  *(HP++) = (ptr) (a); 
#define xFNSHxNODE()  {HDRS->num = HP-(HDRS->sadr);xPUSHA(HDRS--);} 
#define xPUSHM(a)  *(MP++) = (ptr) (a); 
#define xPUSHA(a)  {*AP = (ptr) (a) ; AP--;} 
...
#else
void allochandler(int sig, int code, struct sigcontext *sc)
#endif
...
#else
void memhandler(int sig, int code, struct sigcontext *sc)
#endif
{ /* check overflow of memspaces, by checking if at end non-zero items */
#ifdef DEBUG
...
{ int count,rrealf;
  ...
  { rrealf=realf-(NCHARS+SPECIAL_SYMS);
    if (!abb[rrealf]) 
       {count = sprintf(thistext,"a%ld",rrealf); 
        abb[rrealf]=thistext; 
        printf("!%s:",abb[rrealf]);
        if (rrealf)
	  { gel_print_bin(p);}
...
#define xTSTxHEAP(n)
/* end of barm.c */
int nfsyms=9;char *fnames[]={"|","listtype",
"Booleans: \"true\" -> BOOL",
"Booleans: \"false\" -> BOOL",
"Booleans: BOOL \"|\" BOOL -> BOOL",
"Booleans: BOOL \"&\" BOOL -> BOOL",
"Booleans: \"not\" \"(\" BOOL \")\" -> BOOL",
"Booleans: \"(\" BOOL \")\" -> BOOL",
"BOOL"};
...
#define xigel_write2 1525

#define xigel_write_args 1529

#define xigel_read2 1533

#define xigel_read_args 1537

#define xigel_drop 1541

#define xigel_len 1545

#define xigel_get 1549
...
xPUSHA(xf42)
xCONTINUE()
xCASE(xf43,xSWITCH(xAFSYM(xARG(1)),xCASE(xf41,xPUSHM(xf41)

xDROPA(2)
xCONTINUE()
xBREAK())
xCASE(xf42,xPUSHM(xARG(2))
xDROPA(2)
xCONTINUE()
xBREAK())
,))
...
xFNSHxNODE()
xCONTINUE()
xCASE(xf44,xSWITCH(xAFSYM(xARG(1)),xCASE(xf41,xPUSHM(xARG(2))
xDROPA(2)
xCONTINUE()
xBREAK())
xCASE(xf42,xPUSHM(xf42)
...
xFNSHxNODE()
xCONTINUE()
xCASE(xf45,xSWITCH(xAFSYM(xARG(1)),xCASE(xf42,xPUSHM(xf41)

xDROPA(1)
xCONTINUE()
xBREAK())
xCASE(xf41,xPUSHM(xf42)
