...
WHITEREGEX = regex.compile("["+string.whitespace+"]+")
...
def DumpStringWindow(Str, Pos, Offset=15):
    ...
    print `Str[start:Pos]`+"*"+`Str[Pos:end]`
  ...
  def Token(self, String, StartPosition):
     ...
     while not finished:
        ...
        for commentexpr in self.commentpatterns:
           ...
           if offset != -1:
              if offset<1:
                 DumpStringWindow(String,StartPosition)
                 raise LexTokenError, "zero length comment"
              commentfound = 1
        ...
        for (regexpr, Flag, Function) in self.regexprlist:
		 ...
		 (value, offset) = Function(String[StartPosition:], offset)
              else:
                 value = matchstring
              return ((Flag, value) , offset + totalOffset)
        if not (commentfound or whitespacefound):
           DumpStringWindow(String,StartPosition)
           raise LexTokenError, "Lexical parse failure"
     #endwhile
  #enddef
...
class KeywordDict:
  ...
  def hasPrefix(self,String,StartPosition,punctuationlist):
     ...
     if self.FirstcharDict.has_key(First):
        Keylist = self.FirstcharDict[First]
...
class LexStringWalker:
  ...
  def DUMP(self):
    DumpStringWindow(self.String,self.Position)
  ...
  def next(self):
    if self.Done:
       self.DUMP()
       raise LexTokenError, "no next past end of file"
    elif self.PastEOF:
       self.Done=1
       ...
       if self.NextPosition <= self.Position:
          self.DUMP()
          raise LexTokenError, "Lexical walker not advancing"
       self.Position = self.NextPosition
...
class ParserObj:
  ...
  def DoOneReduction(self):
    ...
    while flag == MOVETOFLAG:
       ...
       if not Stream.more():
          Stream.DUMP()
          raise EOFError, 'end of stream during parse'
...
#       self.Stack = Stack #not needed, unless stack rep changes
  ...
  def GotoState(self, rule):
     ...
     Stack = self.Stack
     Head = rule.Nonterm
     if Stack[-1]==0: #Stack.IsEmpty():
        BeforeState = FSM.initial_state
     else:
        BeforeState = Stack[Stack[-1]][0] #Stack.Top()[0]
     # is this right? if the stack is empty and the Head
     # is the root nonterm, then goto is final state
     if Stack[-1]==0 and Head == FSM.root_nonTerminal:#Stack.isEmpty()
        ...
        if flag != MOVETOFLAG:
           self.ParseError(BeforeState, Head)
     return Result

  def ParseError( self, State, Token):
  ...
  def GO(self):
    ...
    return self.Stack[self.Stack[-1]][1] #self.Stack.Top()[1]
...
class ParseRule:
  ...
  def reduce(self, Stack, Context=None):
     ...
     for i in range(1,Blength+1):
        ...
        PoppedValue = Stack[Stack[-1]] #Stack.Top()
        #print PoppedValue,
        Stack[-1] = Stack[-1]-1 #Stack.Pop()
