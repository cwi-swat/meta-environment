...
defining the macro ``\txttt{-DDEBUG}'' during compilation:
...
(Python) modulename (without extension \txttt{.py}):
...
function \txttt{require(mod, fun, args)} defined in the adapter
...
\}
\nwused{\\{NWpytJ-pytH-1}}\nwendcode{}\nwbegindocs{32}\nwdocspar
The \txttt{get\_module} function returns a pointer to a newly created
string containing the modulename part of a full filename (without
directory and extension \txttt{.py}).  Example: the filename {\tt
...
\}
\nwused{\\{NWpytJ-pytH-1}}\nwendcode{}\nwbegindocs{34}\nwdocspar
The \txttt{get\_path} function returns a pointer to a newly created
string containing the pathname part of a full filename of a Python
module (without modulename and extension \txttt{.py}). Example: the
filename \txttt{/dir/sub/module.py} has pathname ``/dir/sub/''.
...
\begin{itemize}
\item \txttt{python-adapter-error(\emph{Msg})}: in that case the included
  error message is printed to the screen.
\item \txttt{snd-disconnect(\emph{Trm})}: in that case the complete term is
...
\begin{itemize}
\item \txttt{rec-monitor(\emph{Trm})}: in that case the Python
  ...
  \begin{itemize}
  \item \emph{process creation}: then \txttt{create\_proc(\emph{ProcId},
      \emph{ProcName})} in the user module is called.
  \item \emph{tool creation}: then \txttt{create\_tool(\emph{ToolId},
      \emph{ToolName})} in the user module is called.
  \item \emph{process to tool communication}: then 
    \txttt{proc\_tool\_comm(\emph{ToolId}, \emph{ProcId})} in the
    user module is called.
  \item \emph{tool to process communication}: then 
    \txttt{tool\_proc\_comm(\emph{ProcId}, \emph{ToolId})} in the
    user module is called.
  \item \emph{process to process communication}: then 
    \txttt{proc\_proc\_comm(\emph{ProcId1}, \emph{ProcId2})} in the
    user module is called.
  ...
  \item \emph{update the list of subscribtions of a process}: then 
    \txttt{update\_subs(\emph{ProcId}, \emph{Subs})} in the user
    module is called.
  \item \emph{update the list of notes of a process}: then 
    \txttt{update\_notes(\emph{ProcId}, \emph{Notes})} in the user
    module is called.
  \end{itemize}
\item \txttt{rec-do(\emph{Fun}(\emph{Args}))} or
  \txttt{rec-eval(\emph{Fun}(\emph{Args}))}: in that case the
...
\item \txttt{rec-ack-event(\emph{Trm})}: in that case the argument
  \emph{Trm} is translated to a Python argument and
  \txttt{rec\_ack\_event} in the user module is called.
\item \txttt{rec-terminate(\emph{Trm})}: in that case the argument
  \emph{Trm} is translated to a Python argument and
  \txttt{rec\_terminate} in the user module is called.  Finally, the
...
Using the \txttt{ToPython} function \TB\ terms can be send to the Python
...
{\bf Note:} the \txttt{ToPython} function uses a \TB\ library function 
...
in the directory \txttt{your\_\TB\_root/src} and rebuild the \TB\ 
library.
...
The \txttt{PYfun\_name} function converts a \TB\ function symbol to a
...
\}
\nwused{\\{NWpytJ-pytH-1}}\nwendcode{}\nwbegindocs{48}\nwdocspar
The \txttt{PYstring} function converts a \TB\ string to a Python
...
\}
\nwused{\\{NWpytJ-pytH-1}}\nwendcode{}\nwbegindocs{50}\nwdocspar
The \txttt{PYterm} function converts a \TB\ term to a Python
...
\}
\nwused{\\{NWpytJ-pytH-1}}\nwendcode{}\nwbegindocs{52}\nwdocspar
The \txttt{PYterm\_list} function converts a \TB\ term list to a
...
\}
\nwused{\\{NWpytJ-pytH-1}}\nwendcode{}\nwbegindocs{54}\nwdocspar
The \txttt{PYenv} function converts a \TB\ environment to a Python
...
\}
\nwused{\\{NWpytJ-pytH-1}}\nwendcode{}\nwbegindocs{56}\nwdocspar
The \txttt{is\_to\_tool\_comm} and \txttt{is\_from\_tool\_comm}
functions are boolean checks if a communication atom originates from a
tool or from the \TB. The \txttt{bytes\_in\_term} function returns
...
The \txttt{showhelp} function displays the version, a copyright
...
The \txttt{error\_and\_exit} function displays an error message and
...
\}
\nwused{\\{NWpytJ-pytH-1}}\nwendcode{}\nwbegindocs{62}\nwdocspar
The \txttt{at\_exit} function is called automatically whenever the
...
\}
\nwused{\\{NWpytJ-pytH-1}}\nwendcode{}\nwbegindocs{64}\nwdocspar
The \txttt{main} function analyzes the adapter arguments and
