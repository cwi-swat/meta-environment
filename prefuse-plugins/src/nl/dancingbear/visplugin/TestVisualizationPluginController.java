...
public class TestVisualizationPluginController extends TestCase {
    ...
    public void test_vpIsTypeSupported() {
    	assertEquals(m_pluginController.vpIsTypeSupported(m_pureFactory.make("relation([str,str])")),m_pureFactory.make("snd-value(vp-type-supported(<bool>))", new Boolean(true)));
        assertEquals(m_pluginController.vpIsTypeSupported(m_pureFactory.make("relation([int,str])")),m_pureFactory.make("snd-value(vp-type-supported(<bool>))", new Boolean(false)));
    }
}
