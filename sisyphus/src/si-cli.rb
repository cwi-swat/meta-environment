...
class SisyphusClient 
  def initialize(host, config_manager, profile_name, interval, roots, store, verbose, quiet, days, after, before, mailer, command_line)
    ...
    @roots = roots
    @store = store
    @repo_factory = RepositoryFactory.new
    @verbose = verbose
    @quiet = quiet
    @days = days
    @after = after
  ...
  def ensure_log(time)
    ...
    @log = IndentedLogger.new(@log_device)
  ...
  def add_log_line(string)
    ...
    log = SiLog.new(:line => string, :si_host => dbhost)
    log.save
  end

  def do_the_build(session, time)
    config = @config_manager.configuration(@profile_name, time)
    revision_factory = RevisionFactory.new(config.sources.properties['repositories'], 
                                           config.sources.properties['externals'],
                                           @repo_factory, config.user, config.build_dir, time)  
    target_factory = TargetFactory.new(session, config, @log)
    @builder = Builder.new(@store, @log)
    visitor = Visitor.new(target_factory, revision_factory, @roots, @builder, @log)
    visitor.build_roots
  end
...
if __FILE__ == $0 then
  options = SisyphusClientCommandlineParser.parse(ARGV)
  uname = `uname -n`.chomp
  ...
  config_manager = ConfigurationManager.new(svn_host, 
                                            svn_url,
                                            options.user, 
                                            options.password)

  store = DBStore.new(options.dbconf)

  mailer = SessionMailer.new(options.addresses, 
                             options.from,
                             options.smtp_host, 
                             options.smtp_port)

  client = SisyphusClient.new(uname, 
                              config_manager, 
