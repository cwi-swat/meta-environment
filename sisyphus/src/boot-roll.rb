...
def checkout_all(roots, revision_factory)
  todo = roots
  done = []
  graph = {}
  while todo != [] do
...
def component_deps(locations, graph)
  deps = {}
  graph.each_key do |r|
...
def make_roll_configuration(graph)
  conf = {}
...
def obtain_checkouts(conf, roots, options)
  s = conf['sisyphus']
  config_manager = nil
  if s.has_key?('tunnel') then
    config_manager = ConfigurationManager.new(s['host'],
                                              s['url'],
                                              s['user'],
                                              s['password'],
                                              s['port'],
  ...
  revision_factory = RevisionFactory.new(repositories, 
                                         externals,
