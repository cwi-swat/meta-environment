...
if $0 == __FILE__ then
  options = Updating::CommandLineParser.parse(ARGV)
  if ARGV == [] then
    Updating::CommandLineParser.parse(['-h'])
  end
  pkg = ARGV[0]
  log = Logger.new(STDERR)
  log.level = Logger::ERROR
  if options.verbose then
    log = Updating::MyLog.new
  end
  dist_conf = Distribution::DistributionConfiguration.new(options.url)
