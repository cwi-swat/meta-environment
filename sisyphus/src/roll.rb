...
class CommandlineParser
  def self.read_roll_conf
    if not File.exists?(roll_rcfile) then
      $stderr << "File #{roll_rcfile} does not exist. Run boot-roll first.\n"
      exit(1)
    end
    conf = nil
    File.open(roll_rcfile) do |f|
      conf = YAML.load(f)
    end
    return conf
  end
  ...
  def self.update_roll_conf(conf)
    $stderr << "File #{boot_roll_rcfile} is newer than #{roll_rcfile}; run boot-roll? [Yn] "
  ...
  def self.checked_package(conf, package)
    if conf['locations'].include?(package)
      ...
      $stderr << "Unknown package #{package}; see #{roll_rcfile}.\n"
      exit(1)
    end
  end
  ...
  def self.the_conf
    ...
    if File.exists?(boot_roll_rcfile) and
        File.ctime(boot_roll_rcfile) > File.ctime(roll_rcfile) then
...
def instantiate(component, template, graph, locations, install_dir)
  deps = []
...
def make_command(action, component, templates, deps, locations, install_dir)
...
def run_commands(actions, locations, deps, templates, install_dir, options)
  pwd = Dir.getwd
  order = deps.tsort
...
if $0 == __FILE__ then
  options, conf = CommandlineParser.parse(ARGV)
  if ARGV == [] then
    $stderr << "Usage: roll <command>\n"
    exit(1)
  end

  roll(conf, options, ARGV)
end
