...
if __FILE__ == $0
  i1 = MockItem.new('aterm', 3443, true)
  i2 = MockItem.new('toolbus-java-adapter', 234, false)
  session = MockSession.new(`uname -n`.chomp, Time.now)
  session.add('aterm', i1)
  session.add('toolbus-java-adapter', i2)

  mail = SessionMailer.new(['storm@cwi.nl'], 'smtp.cwi.nl', 25)
  mail.mail(session)
