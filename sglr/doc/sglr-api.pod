...
=item ATerm B<SGgetTopSymbols>(char *lang);
...
retreived from the parse table database) by that name. 
...
B<SG_FILTER_OFF>() to disable these ad-hoc filters.
