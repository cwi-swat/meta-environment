...
#ifdef NOPOOL
  if((st = SG_Malloc(1, sizeof(stack)))) {
#else
...
#ifdef NOPOOL
  if((nsts = SG_Malloc(1, sizeof(stacks)))) {
#else
...
#ifdef NOPOOL
  if((lk = SG_Malloc(1, sizeof(st_link)))) {
#else
  if((lk = SG_MallocLink())) {
#endif
    lk->tree = t;
...
#ifdef NOPOOL
  if((lks = SG_Malloc(1, sizeof(st_links)))) {
#else
