...
*/
int 
next_token(int(*get_next_char)(void))
{
  int c;
  c = get_next_char();
  text_length++;
...
*/
path *
conc_paths(paths *ps1, paths *ps2)
{
  path *ps;
...
*/
path *
find_paths(stack *st, int i, st_link *l0, bool link_seen, term *sons)
{
  st_links *ls = NULL;
  st_link  *l1 = NULL;
  term *paths = NULL;
...
void reducer(stack *, state, label, term_list *, bool);
...
*/
term *
parse(parse_table *ptable, int(*get_next_char)(void))
{
  init_input(); 
  table = ptable;
  ...
  do 
    {
      if(show_stack) stack_to_dotfile(active_stacks);
      current_token = next_token(get_next_char);
      parse_character();
      shifter();
      if(gc) TBcollect();
      if(show_stack) stack_to_dotfile_end();
    }
  while (current_token != eof && active_stacks != NULL);
    
  if(show_stack) stack_to_dotfile(mk_list(accepting_stack, NULL));
  if(show_stack) stack_to_dotfile_end();
  return result();
} 
/*
...
{ 
  ...
  while(pop(a, as))
    switch(A_KIND(a)) {
    case SHIFT: add_shift_pair(st, A_STATE(a), for_shifter);
      break;
    case REDUCE: do_reductions(st, a);
      break;
    case ACCEPT: accepting_stack = st;
      break;
    case ERROR:
      break;
    } 
...
*/
void 
do_reductions(stack *st, action *a)
{
  path  *p; 
  paths *ps;
  label prod; 
...
/*
  ...
  Stack |st0| is the stack at the end of path |p|. State |s| is the
  next state when reducing production |r| from the state of
  |st0|. Look for a stack |st1| on |active_stacks| with state |s|.  If
  no such stack is found, create a new stack with state |s| and a link
  to |st0|, which has tree |t| as child. The new stack is added to the
  list of active stacks and to the list of delayed stacks for
  |do_actions|. The delay ensures (?) that all reductions to the same
  stack |st1| occur before any reductions from that state such that a
  rejection of stack |st1| will be detected before any further
  reductions.

  If such a stack exists and there is a direct link |l|
  from |st1| to |st0|, an ambiguity has been found. The
  tree |t| is simply added to the |amb| node of the direct
  link |l|.
...
void
reducer(stack *st0, state s, label prod, term_list *kids, bool reject)
{
  term* t; 
  st_link *nl;
  stack *st1; 
  ...
  if((st1 = find_stack(s, active_stacks)) == NULL) { 
    ...
    if (reject) mark_stack_rejected(st1, nl); 
  }

  /* ambiguity */
  else if((nl = find_direct_link(st1, st0)) != NULL) {
    if(debugflag) 
      TBprintf(log, "direct link found\n");
    amb(TREE(nl), t);
    if (reject) mark_link_rejected1(st1, nl);	  
  }

  /* add new direct link from |st1| to |st0| and
     recheck all reductions all for |st1|. */
  else { 
    stacks *sts = NULL; 
    stack  *st2 = NULL; 
    add_link(st1, t, st0, nl);  
    if (reject) mark_link_rejected2(st1, nl);
    sts = active_stacks;
    while(pop(st2, sts)) { 
      actions *as;
      action *a;
...
*/
void 
do_limited_reductions(stack *st, action *a, st_link *l)
{
  path  *p; 
  paths *ps;
  label prod; 
