...
*/
void
bst_add(term **tree, int first, int last, term *val)
{
...
actions *
ACTIONS(parse_table *pt, state s, int c)
{
  actions *as;
  if (debugflag)
    {
      /* TBprintf(log, "state %d = %t\n\n", s, pt->vertices[s]); */
    }
...
parse_table *
new_parse_table(int states, int productions)
{
  parse_table *pt;
  int i;
...
void
add_goto(parse_table *pt, state s1, int first, int last, state s2)
{
  bst_add(&(pt->goto_table[s1]), first, last, mk_int(s2));
}

void
add_actions(parse_table *pt, state s, int first, int last, actions *as)
{
...
void
bst_add_classes(term ** entry, term *classes, actions *as)
{
  int first, last;
  while(classes != NULL)
    {
...
void
PT_actions(term *action_table, parse_table *pt, state s)
{
...
*/
void
PT_items(term *vertices, parse_table *pt)
{
  term *vertex, *itemset;
  int pr_num;
  int c;
...
void
PT_grammar(term *grammar, parse_table *pt)
{
  term *production;
  int pr_num;
  int c;
