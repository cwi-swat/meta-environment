...
public class ModulePopupMenu {
	...
	private void showModulePopup(final ATerm type, final String moduleName,
			ATerm buttons, JComponent parent, int x, int y) {
		ATermList list = (ATermList) buttons;
		JPopupMenu menu = makePopup(type, moduleName, list);
		menu.show(parent, x, y);
	}

	private JPopupMenu makePopup(final ATerm type, final String moduleName,
			ATermList list) {
	...
	private void addItem(JPopupMenu menu, ATerm type, String moduleName,
			ATerm button) {
		addItem(0, menu, type, moduleName, button);
	}

	private void addItem(int level, JPopupMenu menu, ATerm type,
			String moduleName, ATerm button) {
		...
		if (!found) {
			...
			if (path.getLength() - 1 == level) {
				addLeafButton(menu, type, moduleName, getLabel(button, level),
						button);
			} else {
	...
	private void addItem(int level, JMenu menu, ATerm type, String moduleName,
			ATerm button) {
		boolean found = addSubMenu(level, type, moduleName, button, menu);
	...
	private boolean findSubMenu(int level, ATerm type, String moduleName,
			ATerm button, MenuElement[] children) {
		boolean found = false;
	...
	private void addLeafButton(JPopupMenu menu, final ATerm type,
			final String moduleName, final String label, final ATerm action) {
		JMenuItem item = makeMenuItem(type, moduleName, label, action);
		menu.add(item);
	}

	private void addLeafButton(JMenu menu, final ATerm type,
			final String moduleName, final String label, final ATerm action) {
		JMenuItem item = makeMenuItem(type, moduleName, label, action);
		menu.add(item);
	}

	private JMenuItem makeMenuItem(final ATerm type, final String moduleName,
			final String label, final ATerm action) {
