...
public class MetaStudio extends JFrame implements UserInterfaceTif, Runnable, ModuleSelectionListener  {
	...
	public void updateList(String str) {
		...
		if (data instanceof ATermAppl) {
			...
			System.out.println("Deprecated use of list with function symbol " + applData.getAFun());
		}
	...
	public void buttonsFound(String buttonType, String moduleName, ATerm buttons) {
		if (buttonType.equals(MODULE_POPUP_BUTTON_TYPE)) {
			...
			addPopupMenuItems(modulePopup, buttonType, moduleName, (ATermList) buttons, factory.makeList());
			modulePopup.show(component, mouseX, mouseY);
		} else if (buttonType.equals(NEW_MODULE_POPUP_BUTTON_TYPE)) {
			newModulePopup.removeAll();
			addPopupMenuItems(newModulePopup, buttonType, moduleName, (ATermList) buttons, factory.makeList());
			...
			if (buttonName.getLength() == 1) {
				menu.add(
					new ButtonAction(
						...
						buttonType,
						prefixButtonName.concat(buttonName),
						moduleTree,
						bridge,
						factory));
			} else {
				...
				addMenuItems(
					...
					moduleName,
					subMenu,
					prefixButtonName.insertAt(buttonNamePrefix, prefixButtonName.getLength()));
				menu.add(nextLevel);
			}
		}
	}
