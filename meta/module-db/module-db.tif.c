 ...
 * Implementation generated at Wed Jan 15 13:49:47 2003
 */

#include "module-db.tif.h"

#define NR_SIG_ENTRIES	37

static char *signature[NR_SIG_ENTRIES] = {
  "rec-do(<module-db>,create-module-db)",
  ...
  "rec-eval(<module-db>,update-eqs-tree(<str>,<term>))",
  "rec-eval(<module-db>,update-eqs-text(<str>,<str>))",
...
ATerm module_db_handler(int conn, ATerm term)
{
  ...
  if(ATmatch(term, "rec-eval(update-eqs-tree(<str>,<term>))", &s0, &t0)) {
    return update_eqs_tree(conn, s0, t0);
  }
  if(ATmatch(term, "rec-do(add-tree-eqs-section(<str>,<str>,<term>,<str>,<int>))", &s0, &s1, &t0, &s2, &i0)) {
    add_tree_eqs_section(conn, s0, s1, t0, s2, i0);
    return NULL;
  }
  if(ATmatch(term, "rec-eval(update-eqs-text(<str>,<str>))", &s0, &s1)) {
    return update_eqs_text(conn, s0, s1);
  }
  ...
  if(ATmatch(term, "rec-do(create-module-db)")) {
    create_module_db(conn);
    return NULL;
  }
