...
public class FactBrowser extends DefaultStudioPlugin implements FactbrowserTif,
     ...
     * identifiers from the dataManager. Then it sends it over the toolbus.
     * This method handles the double clicking on tree nodes and also the
     * right mouse clicking on rstore nodes so users can unload rstores.
     * Currently no Mac support is build in.
     * 
     * @param mouseEvent
     ...
     * @author Anton Lycklama a Nijeholt
     * 
     * In the future there needs to be Mac support.
     * This isn't fully implemented yet but it should
     ...
     * Only the messages will be send, the data itself will not be handled here.
     * 
     * Scenario 1:
     * If the user double clicks on a rstore tree node a message will be send to the toolbus
     * and requests all the facts within the rstore.
     * 
     * Scenario 2:
     * If the user double clicks on a fact tree node a message will be send to the toolbus
     * to request all the possible visualizations for the given fact. 
     *  
     * @author Renze de Vries
     * @author Bas Basten
     * @author Anton Lycklama a Nijeholt
    ...
    private void mouseDoubleClick(MouseEvent mouseEvent) {
        factBrowserWindow.selectNodeAtPosition(mouseEvent.getX(), mouseEvent.getY());
        ...
        if (selectedNode != null) {
            ...
            if (dataManager.checkValidVisualisationPlugin(selectedNode)) {
                ...
                ATerm term = metaStudio.getATermFactory().make(
                        ...
                        new Integer(rstoreID), new Integer(factID), new Integer(visPluginID));
    ...
    /**
     * Sends an unload message when the unload popup menu button has been clicked.
     * 
     * @author Bas Basten
     * @author Anton Lycklama a Nijeholt
     * @date 19-03-2007
    ...
    public void actionPerformed(ActionEvent actionEvent) {
        if (actionEvent.getSource() == m_unloadMenu) {
            ...
            DefaultMutableTreeNode selectedNode = factBrowserWindow.getSelectedNode();
            
            if (dataManager.checkValidRStore(selectedNode)) {
                int rstoreID = dataManager.getRStoreID(selectedNode);
             
                // Build the Aterm with RStore FactType
                ATerm term = metaStudio.getATermFactory().make("fb-unload-rstore(<int>)", new Integer(rstoreID));
                factBrowserBridge.sendEvent(term);
            }
        }
    }
