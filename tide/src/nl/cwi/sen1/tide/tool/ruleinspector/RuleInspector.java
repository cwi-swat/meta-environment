...
public class RuleInspector extends ProcessTool implements DebugAdapterListener {
...
class RuleSelector extends JPanel implements DebugProcessListener,
	...
	public RuleSelector(DebugProcess process) {
		...
		listeners = new LinkedList<RuleSelectionListener>();
	...
	public void ruleCreated(DebugProcess process, Rule rule) {
		...
		tableModel.fireTableChanged(new TableModelEvent(tableModel, index,
	...
	public void ruleDeleted(DebugProcess process, Rule rule) {
		...
		tableModel.fireTableChanged(new TableModelEvent(tableModel, index,
				index, TableModelEvent.ALL_COLUMNS, TableModelEvent.DELETE));
	}

	public void ruleModified(DebugProcess process, Rule rule) {
		int index = process.getRuleIndex(rule);
		tableModel.fireTableChanged(new TableModelEvent(tableModel, index,
				index, TableModelEvent.ALL_COLUMNS, TableModelEvent.UPDATE));
	}
...
class RuleSelectorTableModel extends AbstractTableModel {
	private DebugProcess process;
...
class RuleEditor extends JPanel implements RuleSelectionListener,
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (source == enabled) {
			...
			// Handle create rule
			String tag = (String) JOptionPane.showInternalInputDialog(this,
					"What tag should the new rule have?", "Create a new Rule",
					JOptionPane.PLAIN_MESSAGE, null, null, "manual-rule");
			if (tag != null) {
				process.requestRuleCreation(Port.makeStep(), Expr.makeTrue(),
						Expr.makeTrue(), tag, false);
			}
			...
			if (port == null) {
				JOptionPane.showInternalMessageDialog(this, "Illegal port: "
						+ portItem, "Illegal Port", JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			if (cond == null) {
				JOptionPane.showInternalMessageDialog(this,
						"Not an expression: " + condTxt, "Illegal Condition",
			...
			if (act == null) {
				JOptionPane.showInternalMessageDialog(this,
						"Not an expression: " + actTxt, "Illegal Action",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
