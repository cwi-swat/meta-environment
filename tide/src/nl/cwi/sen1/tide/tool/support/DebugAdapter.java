...
public class DebugAdapter {
	...
	public DebugAdapter(DebugTool tool, ATerm dap) {
		...
		listeners = new LinkedList<DebugAdapterListener>();
	...
	public void event(int pid, int rid, ATerm result) {
		/*
		 * Set keys = processes.keySet(); Collection values =
		 * processes.values();
		 * 
		 * Iterator itkeys = keys.iterator(); Iterator itvalues =
		 * values.iterator();
		 * 
		 * 
		 * System.out.println("<---------------- process listing (dap =
		 * "+this.getName()+") ------------------>"); while (itkeys.hasNext()) {
		 * System.out.println("--- key: "+itkeys.next()); } while
		 * (itvalues.hasNext()) { DebugProcess procje = (DebugProcess)
		 * itvalues.next(); System.out.println("--- val: "+procje.getName()); }
		 */
		DebugProcess proc = findProcess(pid);
		/*
		 * if (proc==null) { System.out.println("shiiiiiiiiiiiiiiiiiiiiiiiiiiiit
		 * geen proc\n"); } System.out.println("<-------------------------------------------------------->"); //
		 * System.out.println("[DebugAdapter] event() process:
		 * "+proc.getName()+", rid: "+rid+", result: "+result);
		 */
	...
	// }}}
	// {{{ public void ruleModified(int pid, int rid, port, cond, action,
	// enabled)

	public void ruleModified(int pid, int rid, Port port, Expr cond,
			Expr action, boolean enabled) {
		DebugProcess process = findProcess(pid);
	...
	public void requestRuleCreation(int pid, Port port, Expr cond, Expr act,
	...
	// }}}
	// {{{ public void requestRuleModification(pid, rule, port, cond, act,
	// enabled)

	public void requestRuleModification(int pid, Rule rule, Port port,
