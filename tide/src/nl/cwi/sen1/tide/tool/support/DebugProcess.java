...
public class DebugProcess {
	...
	public DebugProcess(DebugAdapter adapter, int pid, String name) {
		...
		statusChangeListeners = new LinkedList<ProcessStatusChangeListener>();
		processListeners = new LinkedList<DebugProcessListener>();

		requestRuleCreation(Port.makeStarted(), Expr.makeTrue(), Expr
				.makeTrue(), tag_started);
	...
	public void addProcessStatusChangeListener(
			ProcessStatusChangeListener listener) {
		statusChangeListeners.add(listener);
	}

	public void removeProcessStatusChangeListener(
			ProcessStatusChangeListener listener) {
		statusChangeListeners.remove(listener);
	}

	public void fireProcessStatusChanged() {
		Iterator<ProcessStatusChangeListener> iter = statusChangeListeners
				.iterator();
	...
	public void fireRuleCreated(Rule rule) {
		Iterator<DebugProcessListener> iter = ((List<DebugProcessListener>) ((LinkedList<DebugProcessListener>) processListeners)
				.clone()).iterator();
	...
	public void fireRuleModified(Rule rule) {
		Iterator<DebugProcessListener> iter = ((List<DebugProcessListener>) ((LinkedList<DebugProcessListener>) processListeners).clone())
				.iterator();
	...
	public void ruleModified(int rid, Port port, Expr cond, Expr action,
			boolean enabled) {
		Integer ridKey = new Integer(rid);
	...
	public void requestRuleCreation(Port port, Expr cond, Expr act, String tag) {
	...
	public void requestRuleModification(Rule rule, Port port, Expr cond,
	...
	public void requestRuleEnabling(Rule rule, boolean enabled) {
		requestRuleModification(rule, rule.getPort(), rule.getCondition(), rule
				.getAction(), enabled);
	}
