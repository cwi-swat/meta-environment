...
abstract public class DebugToolTool extends SwingTool implements DebugToolTif {
	...
	private void initSigTable() {
		...
		sigTable
				.add(factory
						.parse("rec-do(<debug-tool>,adapter-connected(<debug-adapter>))"));
		sigTable
				.add(factory
						.parse("rec-do(<debug-tool>,adapter-disconnected(<debug-adapter>))"));
		sigTable
				.add(factory
						.parse("rec-do(<debug-tool>,process-created(<proc(<debug-adapter>,<int>)>,<str>))"));
		sigTable
				.add(factory
						.parse("rec-do(<debug-tool>,process-destroyed(<proc(<debug-adapter>,<int>)>))"));
		sigTable
				.add(factory
						.parse("rec-do(<debug-tool>,event(<proc(<debug-adapter>,<int>)>,<int>,<term>))"));
		sigTable.add(factory.parse("rec-terminate(<debug-tool>,<term>)"));
		sigTable.add(factory
				.parse("rec-eval(<debug-adapter>,evaluate(<int>,<term>))"));
		sigTable
				.add(factory
						.parse("rec-eval(<debug-adapter>,create-rule(<int>,<term>,<term>,<term>,<term>,<term>))"));
		sigTable.add(factory
				.parse("rec-eval(<debug-adapter>,enable-rule(<int>,<int>))"));
		sigTable.add(factory
				.parse("rec-eval(<debug-adapter>,disable-rule(<int>,<int>))"));
		sigTable
				.add(factory
						.parse("rec-eval(<debug-adapter>,modify-rule(<int>,<int>,<term>,<term>,<term>,<term>))"));
		sigTable.add(factory
				.parse("rec-eval(<debug-adapter>,delete-rule(<int>,<int>))"));
	...
	private void initPatterns() {
		...
		PadapterDisconnected0 = factory
				.parse("rec-do(adapter-disconnected(<term>))");
		Pevent0 = factory.parse("rec-do(event(<term>,<int>,<term>))");
		PprocessCreated0 = factory
				.parse("rec-do(process-created(<term>,<str>))");
	...
	public ATerm handler(ATerm term) {
		...
		if (result != null) {
			event((ATerm) result.get(0), ((Integer) result.get(1)).intValue(),
					(ATerm) result.get(2));
			return null;
		}
