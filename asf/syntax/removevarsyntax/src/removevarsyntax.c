...
static char myarguments[] = "bhm:i:o:tvV";   
...
/*}}}  */
/*{{{  ATerm add_removevarsyntax_function_standalone(ATerm term) */
...
ATerm add_removevarsyntax_function_standalone(ATerm term)
{
  ...
  if(ATmatchTerm(term, pattern_asfix_term,
    ...
    t_name = ATparse("l(\"remove-var-syntax\")");
    ...
    result = ATmakeTerm(pattern_asfix_appl,
                      ATparse("prod(id(\"Remove-Var-Syntax\"),w(\"\"),[ql(\"remove-var-syntax\"),w(\"\"),ql(\"(\"),w(\"\"),sort(\"SDF\"),w(\"\"),ql(\")\")],w(\"\"),l(\"->\"),w(\"\"),sort(\"SDF\"),w(\"\"),no-attrs)"),
                      term_ws,
...
/*}}}  */
/*{{{  ATerm add_removevarsyntax_function(char *str, ATerm term) */
...
ATerm add_removevarsyntax_function(char *str, ATerm term)
{
  ...
  if(ATmatchTerm(term, pattern_asfix_term,
    ...
    result = ATmakeTerm(pattern_asfix_appl,
                      ATparse("prod(id(\"Remove-Var-Syntax\"),w(\"\"),[ql(\"remove-var-syntax\"),w(\"\"),ql(\"(\"),w(\"\"),sort(\"ModuleName\"),w(\"\"),ql(\",\"),w(\"\"),sort(\"SDF\"),w(\"\"),ql(\")\")],w(\"\"),l(\"->\"),w(\"\"),sort(\"SDF\"),w(\"\"),no-attrs)"),
                      term_ws,
...
ATerm remove_var_syntax(int cid, char *name, ATerm t)
{
  ...
  et = AFexpandTerm(t);
  ft = add_removevarsyntax_function(name, et);
...
{
    ATwarning(
        "Usage: %s -bh -m file -i file -o file -tvV . . ."
        "Options:\n"
        "\t-b              output terms in BAF format (default)\n"
        "\t-h              display help information (usage)\n"
	"\t-m module       topmodule of the specification (default: Main)\n"
...
int main(int argc, char *argv[])
{
  ...
  if(toolbus_mode) {
      ...
      ATBeventloop();
    #else
      ATwarning("removevars: Toolbus cannot be used in Windows.\n");
    #endif
  }
  else {
    ...
    if(proceed) {  
      ...
      if(!strcmp(moduleName,"")) {
        et = add_removevarsyntax_function_standalone(t);
      } else {
      	et = add_removevarsyntax_function(moduleName,t);
      }
