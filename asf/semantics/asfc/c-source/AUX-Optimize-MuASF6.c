...
ef4 = lookup_func ( ATreadFromString ( "prod(id(\"Optimize-MuASF\"),w(\"\"),[ql(\"remove-only-in-recursive-args\")],w(\"\"),l(\"->\"),w(\"\"),sort(\"RemoveOption\"),w(\"\"),no-attrs)" ) ) ;
ef4sym = lookup_sym ( ATreadFromString ( "prod(id(\"Optimize-MuASF\"),w(\"\"),[ql(\"remove-only-in-recursive-args\")],w(\"\"),l(\"->\"),w(\"\"),sort(\"RemoveOption\"),w(\"\"),no-attrs)" ) ) ;
...
if ( not_empty_list ( tmp [ 2 ] ) ) {
( tmp [ 3 ] = ( * ef5 ) ( atmp02 , arg1 ) ) ;
FUNC_EXIT ( ( * ef6 ) ( make_nf1 ( ef2sym , lf2 ( make_list ( tmp [ 2 ] ) ) ) , atmp01 , tmp [ 3 ] ) ) ;
}
}
}
}
