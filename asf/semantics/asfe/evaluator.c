...
ATerm args_matching(ATerm env,ATermList conds,
		    ATermList args1,ATermList args2, ATerm lhs_posinfo, int depth);
...
ATerm conds_satisfied(ATermList conds, ATerm env, int depth)
{
  ...
  if(!ATisEmpty(conds)) {
	...
	TIDE_STEP(ATgetAnnotation(lhs, label_pos_info), newenv, depth);
        lhstrm = rewrite(lhs, newenv, depth+1);
	TIDE_STEP(ATgetAnnotation(rhs, label_pos_info), newenv, depth);
        if(no_new_vars(rhs,newenv)) {
          rhstrm = rewrite(rhs, newenv, depth+1);
	  TIDE_STEP(ATgetAnnotation(cond, label_pos_info), newenv, depth);
    ...
    */
	...
	ATwarning("Trying equation: %t.\n",entry->tag);
      ...
      env = args_matching((ATerm) ATempty, conds, equargs, termargs,
			  ATgetAnnotation(entry->lhs, label_pos_info), depth);
      if(!is_fail_env(env)) {
	TIDE_STEP(ATgetAnnotation(entry->rhs, label_pos_info), env, depth);
	if(run_verbose) {
...
ATermList rewrite_elems(ATerm sym, ATermList elems, ATerm env, int depth)
{
  ...
  if(len >= 32) {
	...
	if(keep_whitespace && v_is_empty_slice(newelem)) {
...
int main(int argc, char *argv[])
{
  ...
  ATprotect(&label_pos_info);
  label_pos_info = ATmake("pos-info");
