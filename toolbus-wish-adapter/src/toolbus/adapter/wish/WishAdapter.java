...
public class WishAdapter extends AbstractTool{
	...
	private void dumpArgument(ATerm arg) throws IOException{
		if(arg.getType() == ATerm.LIST){
			dumpListArgument((ATermList) arg);
		}else{
			wishInputStream.write(arg.toString().getBytes());
		}
	}

	private void dumpListArgument(ATermList list) throws IOException{
		if(list.isEmpty()){
		...
		public void run(){
			...
			try{
				while((line = reader.readLine()) != null){
					...
					// The operation has already been interned, so match
					// pointers instead of using equals.
